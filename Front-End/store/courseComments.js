...
export const useCourseCommentsStore = defineStore({
  ...
  actions: {
    async fetchComments(id) {
      ...
      await fetch(
        `http://127.0.0.1:5000/comentarios?token=${token}&target=${target}&id=${id}`,
        {
          method: "GET",
        }
      )
        ...
        .then(async (res) => {
          for (const comment of res.comments) {
            await fetch(
              `http://127.0.0.1:5000/user?token=${token}&target=${target}&type=${comment[3]}&id=${comment[2]}`
            )
              .then((resp) => resp.json())
              .then(async (resp) => {
                await fetch(
                  `http://127.0.0.1:5000/respostas?token=${token}&target=${target}&id=${comment[0]}`
                )
                  ...
                  .then(async (responses) => {
                    ...
                    if (responses.responses.length >= 1) {
                      for (const response of responses.responses) {
                        await fetch(
                          `http://127.0.0.1:5000/user?token=${token}&target=${target}&type=${response[3]}&id=${response[2]}`
                        )
    ...
    async addResponse(comment_id, payload) {
      ...
      const target = localStorage.getItem("_gtt");
      await fetch(
        `http://127.0.0.1:5000/registrarResposta?token=${token}&target=${target}&comment=${comment_id}&mensagem=${payload}`,
        {
          method: "POST",
        }
      )
